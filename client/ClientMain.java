...
public class ClientMain {
	...
	public static void main(String[] args) throws Exception{
		...
		for (int i = 0; i < 10; i++) {
			...
			new Thread() {
				public void run() {
					test2Client(cport, timeout, downloadFolder, uploadFolder, finalI);
				}
			}.start();
		}
	}
	
	public static void test2Client(int cport, int timeout, File downloadFolder, File uploadFolder, int g) {
		Client client = null;
...
//				}
			...
			//try { client.load(list[0]); } catch(IOException e) { e.printStackTrace(); }
	...
	public static void testClient(int cport, int timeout, File downloadFolder) {
		...
		try {
			...
			try { client.store(new File("to_store/someFile.txt")); } catch(IOException e) { e.printStackTrace(); }
